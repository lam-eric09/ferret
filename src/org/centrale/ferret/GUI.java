...
public class GUI extends JFrame{
	...
	public GUI() {
		...
		geneWindowCheckBox.addActionListener(new ActionListener() {
			@Override
			public void actionPerformed(ActionEvent arg0) {
				if (geneWindowCheckBox.isSelected()){
					geneWindowField.setEnabled(true);
				} else {
					geneWindowField.setEnabled(false);
					geneWindowField.setText("");
				}
			}
		});
			...
			public void actionPerformed(ActionEvent arg0) { // Position
				...
				if(inputSelect.getSelectedIndex() == 0){
					...
					boolean fromNCBI = geneNCBIRadioButton.isSelected();
                                        String geneWindowSize = geneWindowField.getText();
					boolean geneWindowSelected = geneWindowCheckBox.isSelected();
					...
					if((geneListInputted || (geneFileImported && !geneFileError && !geneFileExtensionError)) && !invalidCharacter && popSelected && fileLocSelected && !espError){
						...
						final FerretData currFerretWorker = new FerretData(geneQueryType, geneListArray, populations, fileNameAndPath, getESP, progressText, webAddress, mafThreshold[0], ESPMAFBoolean[0] , output,defaultHG[0],geneWindowSelected,Integer.parseInt(geneWindowSize),0);
						currFerretWorker.addPropertyChangeListener(new PropertyChangeListener() {
                ...
                //Treatment of the data and execution of haploview with the adapted files at the end
               goHaplo.addActionListener(new ActionListener() {
                    public void actionPerformed(ActionEvent arg0) { // Position
                        
                        //check if the the created files include HaploView/Plink option
                        if(allFilesButton.isSelected()){
				...
				if(inputSelect.getSelectedIndex() == 0){
					...
					if((snpListInputted || (snpFileImported && !snpFileError && !snpFileExtensionError)) && !invalidCharacter && validWindowSizeEntered && popSelected && fileLocSelected && !espError){
						...
						JOptionPane.showMessageDialog(SNPFerret, errorMessage,"Error",JOptionPane.OK_OPTION);
                            ...
                            StringBuffer errorMessage = new StringBuffer("Correct the following error:");
			    errorMessage.append("\n Select Plink/HaploView files in Settings");
			    JOptionPane.showMessageDialog(SNPFerret, errorMessage,"Error",JOptionPane.OK_OPTION);
                        }
			}
